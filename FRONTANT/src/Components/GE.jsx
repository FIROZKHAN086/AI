  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-[#121212] to-[#1f1f1f] text-white flex flex-col justify-between">
      <div className="max-w-2xl mx-auto w-full px-4 py-6 space-y-4 overflow-y-auto flex-1">
        <h1 className="text-3xl font-bold text-center text-[#facc15] mb-4">AI Assistant</h1>

        {chat.length === 0 ? (
          <p className="text-center text-gray-400">Start a conversation with your voice or type.</p>
        ) : (
          chat.map((item, index) => (
      ...
      <div className="flex justify-center items-center">
        {voice && (
          <img src="/speak.gif" alt="Speak" className="w-16 h-16" />
        )}
        {speak && (
          <img src="/aiVoice.gif" alt="AI Voice" className="w-16 h-16" />
        )}
      </div>
